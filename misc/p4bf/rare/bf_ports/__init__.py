...
class BfPorts(Thread):
    ...
    def sendPortInfoToCP(self):
        ## The list of front-panel ports is generated by traversing
        ## the $PORT_STR_INFO table and picking out all ports that are
        ## a member of the "external_port_list" of the device
        ## configuration table.
        port_str_info_iterator = self.bfgc.port_str_info_table.entry_get(
            self.bfgc.target, None, { "from_hw": False },
            self.bfgc.port_str_info_table.make_data([]),
            p4_name = self.bfgc.p4_name)
        for _hdl, _key in port_str_info_iterator:
            hdl = _hdl.to_dict()
            key = _key.to_dict()
            dev_port = hdl['$DEV_PORT']
            if dev_port in self.bfgc.dev_cfg['external_port_list']:
                data = "portname %s frontpanel-%s\n" % (dev_port, key['$PORT_NAME']['value'])
