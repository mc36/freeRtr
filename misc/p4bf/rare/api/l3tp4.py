...
def writeL3tp4rules(
    self, op_type, nexthop, port, phport, sip, dip, dmac, vrf, smac, tid
):
    if self.tun == False:
        return
    tbl_global_path_1 = "ig_ctl.ig_ctl_tunnel"
    tbl_name_1 = "%s.tbl_tunnel4" % (tbl_global_path_1)
    tbl_action_name_1 = "%s.act_tunnel_l3tp" % (tbl_global_path_1)
    key_field_list_1 = [
        ...
        gc.KeyTuple("hdr.ipv4.protocol", 115),
    ]
    data_field_list_1 = [
    ...
    tbl_action_name_2 = "%s.act_ipv4_l3tp4" % (tbl_global_path_2)
    key_field_list_2 = [
