...
def writeMgre4rules(delete, p4info_helper, ingress_sw, nexthop, port, phport, sip, grp, dip, dmac, vrf, smac):
    table_entry1 = p4info_helper.buildTableEntry(
        table_name="ig_ctl.ig_ctl_tunnel.tbl_tunnel4",
        match_fields={
            "ig_md.vrf": vrf,
...
def writeMgre6rules(delete, p4info_helper, ingress_sw, nexthop, port, phport, sip, grp, dip, dmac, vrf, smac):
    table_entry1 = p4info_helper.buildTableEntry(
        table_name="ig_ctl.ig_ctl_tunnel.tbl_tunnel6",
        match_fields={
            "ig_md.vrf": vrf,
...
def writeEoip4rules(delete, p4info_helper, ingress_sw, bridge, addr, sip, dip, nexthop, instance, vrf, port):
    table_entry1 = p4info_helper.buildTableEntry(
        table_name="ig_ctl.ig_ctl_bridge.tbl_bridge_learn",
        match_fields={
            "ig_md.bridge_id": bridge,
    ...
    table_entry2 = p4info_helper.buildTableEntry(
        ...
        action_name="ig_ctl.ig_ctl_bridge.act_set_bridge_eoip4",
        action_params={
            "nexthop": nexthop,
            "dst_ip_addr": dip,
            "src_ip_addr": sip,
            "instance": (instance >> 8) | ((instance & 0xff) << 8)
        })
    if delete == 1:
        ingress_sw.WriteTableEntry(table_entry2, False)
    elif delete == 2:
    ...
    table_entry3 = p4info_helper.buildTableEntry(
        ...
        action_name="ig_ctl.ig_ctl_tunnel.act_tunnel_eoip",
        action_params={
            "port": port
        })
    if delete == 1:
...
def writeEoip6rules(delete, p4info_helper, ingress_sw, bridge, addr, sip, dip, nexthop, instance, vrf, port):
    table_entry1 = p4info_helper.buildTableEntry(
        table_name="ig_ctl.ig_ctl_bridge.tbl_bridge_learn",
        match_fields={
            "ig_md.bridge_id": bridge,
    ...
    table_entry2 = p4info_helper.buildTableEntry(
        ...
        action_name="ig_ctl.ig_ctl_bridge.act_set_bridge_eoip6",
        action_params={
            "nexthop": nexthop,
            "dst_ip_addr": dip,
            "src_ip_addr": sip,
...
def writePppwheRules(delete, p4info_helper, ingress_sw, port, phport, nexthop, vrf, ses, dmac, smac, core_dst_mac, core_src_mac, label, vpnlab):
    table_entry1 = p4info_helper.buildTableEntry(
        table_name="ig_ctl.ig_ctl_pppoe.tbl_pppoe",
        match_fields={
            "ig_md.source_id": phport,
...
def writeMbierVpnRouRules(delete, p4info_helper, ingress_sw, ipver, vrf, sess, dip, sip, ingr, port, hopid, outlab, vpnlab, subif, bfir, si, bs0, bs1, bs2, bs3, bs4, bs5, bs6, bs7):
    global mcast
    sess = sess & 0xfff
    if delete != 3:
        mcast.append({"egress_port":port, "instance":(hopid+si)})
    table_entry = p4info_helper.buildTableEntry(
        ...
        action_name="eg_ctl.eg_ctl_mcast.act_encap_vpn"+ipver+"_bier",
        action_params={
            "hop": hopid,
            "label": outlab,
            "vpnlab": vpnlab,
...
def main(p4info_file_path, bmv2_file_path, p4runtime_address, freerouter_address, freerouter_port):
    ...
    while 1:
        ...
        if cmds[0] == "pppwhe":
            writePppwheRules(mode,p4info_helper,sw1,int(splt[1]),int(splt[2]),int(splt[3]),int(splt[4]),int(splt[5]),splt[6],splt[7],splt[8],splt[9],int(splt[10]),int(splt[11]))
            continue
        ...
        if cmds[0] == "mgre4":
            writeMgre4rules(mode,p4info_helper,sw1,int(splt[1]),int(splt[2]),int(splt[3]),splt[4],splt[5],splt[6],splt[7],int(splt[8]),splt[9])
            continue

        if cmds[0] == "mgre6":
            writeMgre6rules(mode,p4info_helper,sw1,int(splt[1]),int(splt[2]),int(splt[3]),splt[4],splt[5],splt[6],splt[7],int(splt[8]),splt[9])
            continue
        ...
        if cmds[0] == "bridgeeoip4":
            writeEoip4rules(mode,p4info_helper,sw1,int(splt[1]),splt[2],splt[3],splt[4],int(splt[5]),int(splt[6]),int(splt[7]),int(splt[8]));
            continue

        if cmds[0] == "bridgeeoip6":
            writeEoip6rules(mode,p4info_helper,sw1,int(splt[1]),splt[2],splt[3],splt[4],int(splt[5]),int(splt[6]),int(splt[7]),int(splt[8]));
            continue
        ...
        if cmds[0] == "mbiervpnrou4":
            writeMbierVpnRouRules(mode,p4info_helper,sw1,"4",int(splt[1]),int(splt[2]),splt[3],splt[4],int(splt[5]),int(splt[6]),int(splt[7]),int(splt[8]),int(splt[9]),int(splt[10]),int(splt[11]),int(splt[12]),int(splt[13]),int(splt[14]),int(splt[15]),int(splt[16]),int(splt[17]),int(splt[18]),int(splt[19]),int(splt[20]))
            continue

        if cmds[0] == "mbiervpnrou6":
            writeMbierVpnRouRules(mode,p4info_helper,sw1,"6",int(splt[1]),int(splt[2]),splt[3],splt[4],int(splt[5]),int(splt[6]),int(splt[7]),int(splt[8]),int(splt[9]),int(splt[10]),int(splt[11]),int(splt[12]),int(splt[13]),int(splt[14]),int(splt[15]),int(splt[16]),int(splt[17]),int(splt[18]),int(splt[19]),int(splt[20]))
            continue
