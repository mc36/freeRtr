...
if {[string length $auth] > 0} {
 config "server dns inet" "no zone $domain rr $auth.$domain $prot $clnt"
 config "server dns inet" "zone $domain rr $auth.$domain $prot $clnt"
 }
puts "<html><head><title>dyndns</title></head>"
