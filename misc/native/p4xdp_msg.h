...
int doOneCommand(unsigned char* buf) {
    ...
    if (strcmp(arg[0], "neigh4") == 0) {
        ...
        if (del == 0) {
            ...
            if (bpf_map_delete_elem(neighs_fd, &neik) != 0) warn("error removing entry");
        } else {
            if (bpf_map_update_elem(route4_fd, &rou4, &rour, BPF_ANY) != 0) warn("error setting entry");
            if (bpf_map_update_elem(neighs_fd, &neik, &neir, BPF_ANY) != 0) warn("error setting entry");
        }
        return 0;
    }
    if (strcmp(arg[0], "neigh6") == 0) {
        inet_pton(AF_INET6, arg[3], buf2);
        neik.vrf = rou6.vrf = atoi(arg[5]);
