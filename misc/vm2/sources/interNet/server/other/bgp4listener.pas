...
Var
  ...
  ab0:byte absolute a;
Begin;
move(dat.adr,ab[1],sizeof(dat.adr));
ab0:=sizeof(dat.adr);
a:=IPv4addressPrefix+a;
convertNLRI:=ipAddr2string(ab[1])+'/'+BStr(dat.len);
End;
...
for i:=1 to dataNum do begin;
  if (dataBuf[i].adr<>dat.adr) then continue;
...
a:=chr(orgn);add($4001);
a:=aspt;add($4002);
ab0:=4;move(localAddr[13],a[1],ab0);add($4003);
ab0:=4;writeLongMSB(a[1],med);if (med<>0) then add($8004);
ab0:=4;writeLongMSB(a[1],prf);if (prf<>0) then add($4005);
putAttrs:=b;
End;
