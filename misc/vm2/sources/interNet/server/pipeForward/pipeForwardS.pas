...
Var
  ...
  a:String;
BEGIN;
WriteLn('pipe forward server v1.0, done by Mc at '#%date' '#%time'.');
TCPfindProcess;
...
if (q=0) then begin;
  writeln('using: pipeForwardS.code <port> <process> [0/1=blockmode]');
  exit;
  end;
...
i:=BVal(ParamStr(3));
if (pipeLineCreate(q,q,65536,i<>0)<>0) then begin;
