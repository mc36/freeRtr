...
Var
  ...
  a,b:String;
BEGIN;
WriteLn('websense server v1.0, done by Mc at '#%date' '#%time'.');
...
a:=paramStr(1);
if (a='') then immErr('using: websense.code <config>');
ReadUpConfig(a);

if (serverPort=0) then serverPort:=15868;
i:=serverPort;
