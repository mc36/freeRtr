...
{$include \sources\internet\kernel\tls\cert.inc}
{$include \sources\internet\kernel\tls\tls1.inc}
{$include dtls1.inc}
{$include dtls2.inc}
{$include config.inc}
...
Var
  ...
  a,b:String;
BEGIN;
WriteLn('dtls server v1.0, done by Mc at '#%date' '#%time'.');
Randomize;
if UDPfindProcess then immErr('failed to find udp process!');
...
a:=ParamStr(1);
if (a='') then immErr('using: dtls.code <config>');
ReadUpConfig(a);
