...
Var i,o:LongInt;
BEGIN;
WriteLn('stun client v1.0, done by Mc at '#%date' '#%time'.');
if TCPfindProcess then immErr('failed to find tcp process!');

if string2ipAddr(ParamStr(1),prAdr) then immErr('using: stun.code <host> [port] [group]');
prPrt:=BVal(ParamStr(2));
if (prPrt=0) then prPrt:=1994;
