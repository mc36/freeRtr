...
Var
  ...
  t:xtText;
BEGIN;
WriteLn('rsa req signer v1.0, done by Mc at '#%date' '#%time'.');
Randomize;
...
if (paramStr(3)='') then immErr('using: signRSAreq.exe <key> <person> <req> <output> [days]');

WriteLn('going to sign request with RSA key...');
...
ExportCertificateData(ASN1objectId_RSAwithMD5,a,b,false,0,buf4,buf1,buf2,buf3);
buf1:=buf3;
