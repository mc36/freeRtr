...
Var
  ...
  t:xtText;
BEGIN;
WriteLn('rsa cert creator v1.0, done by Mc at '#%date' '#%time'.');
Randomize;
...
if (paramStr(3)='') then immErr('using: mkssRSAcert.exe <key> <person> <output> [days]');

WriteLn('going to create self-signed RSA certificate...');
...
ExportCertificateData(ASN1objectId_RSAwithMD5,a,b,true,0,buf1,buf1,buf2,buf3);
buf1:=buf3;
