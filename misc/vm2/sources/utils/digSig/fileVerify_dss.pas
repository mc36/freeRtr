...
Var
  ...
  f:xFile;
BEGIN;
WriteLn('dss file verify v1.0, done by Mc at '#%date' '#%time'.');
Randomize;
...
if (paramStr(3)='') then immErr('using: vrfyDSSfile.exe <key> <file> <signature>');

WriteLn('going to verify file with DSS key...');
...
if verifyDataWithDSS(key,buf1,buf2) then immErr('signature error!');
