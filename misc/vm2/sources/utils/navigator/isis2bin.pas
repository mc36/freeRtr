...
i:=pos('/',b);
if (i=0) then exit;
if (copy(b,i+2,1)<>'/') then exit;
i:=pos('-',b);
if (i<1) then exit;
extractAdverter:=copy(b,1,i-1);
End;
...
Begin;
extractLink:='';
if (getNextWord(b,':')<>'metric') then exit;
i:=BVal(getNextWord(b,' '));
a:=getNextWord(b,' ');
if (a<>'is') and (a<>'is-extended') then exit;
a:=getNextWord(b,' ');
a:='1234'+a;
move(i,a[1],sizeof(i));
extractLink:=a;
...
Var
  ...
  md:oneNameRecord;
BEGIN;
writeLn('isis2bin v1.0, done by Mc at '#%date' '#%time'.');
a:=paramStr(1);
