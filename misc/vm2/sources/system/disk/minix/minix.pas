...
Var
  ...
  i,o,p,q:LongInt;
BEGIN;
WriteLn('minix filesystem v1.0, done by Mc at '#%date' '#%time'.');
Randomize;
if (paramCount<3) then begin;
  WriteLn('using: minix.code <process> <drive> <letter>');
  Halt(1);
  end;
a:=ParamStr(2);
i:=BVal(a);
...
if ReadUpBootSector then immErr('this is not a minix drive!');
...
case i of
  ...
  $05:begin;{create file}
    ...
    if (StreamWrite(strm1,sizeof(ntry),ntry)=0) then goto f2;
    if StreamClose(strm1) then goto f2;
    DriveComm.cmd:=0;
    end;
  $06:begin;{erase file}
    ...
    if DeleteOneDirOnlyEntry(strm1,GetFileNameFromEntry(ntry)) then goto f2;
    DriveComm.cmd:=0;
    end;
  ...
  $07:begin;{rename}
    ...
    strm2.pos:=strm2.siz;
    if (StreamWrite(strm2,sizeof(ntry2),ntry2)=0) then goto f2;
    p:=strm2.inoNum;
