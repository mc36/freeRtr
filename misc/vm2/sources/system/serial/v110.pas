...
a:=encodeQueu;
o:=(sizeof(a)-ab0) shr 1;
if (o>64) then pipeLineRecv(upPipeData,ab[1],o) else o:=0;
ab0:=o;
encodeQueu:=encodeQueu+a;
if (o<1) then begin;
...
if (o>addrSiz) then begin;
  decodeInterBytes(buf[addrSiz+1],o-addrSiz);
  end;
f1:
if (encodeBufS>sizeof(encodeBufD) shr 1) then goto f2;
if (encodeQueu<>'') then begin;
  encodeOneRow;
  if (encodeQueu='') then encodeAsyncNone(true);
  goto f1;
  end;
if (encodeBytS>0) then begin;
  encodeOneRow;
  goto f1;
  end;
f2:
...
Var
  ...
  i,o:LongInt;
BEGIN;
WriteLn('v110 v1.0, done by Mc at '#%date' '#%time'.');

i:=BVal(paramStr(2));
if (i<1) then immErr('using: v110.code <process> <speed>');
initializeRate(i);
if (intermediateMeth<1) then immErr('invalid speed!');
a:=paramStr(1);
devPipe:=BVal(a);
if (devPipe=0) then devPipe:=BugOS_findProcNam(a);
if (devPipe<1) then immErr('process not found!');
...
myName:='v110 on '+copy(a,1,pos(#0,a)-1);
