...
Var
  ...
  ioBase:LongInt;
  memBase:LongInt;
  delBeg,delSiz,delEnd:LongInt;
  delDat:array[1..512] of byte;
...
BEGIN;
WriteLn('del v1.0, done by Mc at '#%date' '#%time'.');

a:=ParamStr(1);
delBeg:=BVal(ParamStr(2));
delSiz:=BVal(ParamStr(3));
delEnd:=delBeg+delSiz;
if (delSiz<1) then immErr('using: del.code <process> <beg> <siz>');
WriteLn('process: '+a);
...
if (pipeLineRecv(etherPipe,buf,o)=0) then begin;
  dec(o,delSiz);
  move(buf[delBeg+1],delDat,delSiz);
  move(buf[delEnd+1],buf[delBeg+1],o);
...
if (upperPipe=0) then begin;
  ...
  upperPipe:=p;
  i:=addrSiz;
  if (delBeg<i) then dec(i,delSiz);
  if (i<1) then i:=0;
  move(i,buf[1],sizeof(addrSiz));
  i:=packSiz-2;
...
move(buf[delBeg+1],buf[delEnd+1],p);
move(delDat,buf[delBeg+1],delSiz);
pipeLineSend(etherPipe,buf,p+delSiz);
goto f1;
END.