...
if (a='jmpc') then begin;
  ...
  a:=getWord(c);
  if (a='') then exit;
  d:='j'+CondCods[CompareSeenSign][i]+' near '+a;
  goto vege;
  end;
...
if (a='procallocbeg') then begin;
  ...
  d:=getWord(c);
  if (d='') then exit;
  outputLine('push dword [procTable_data+'+a+'*4]',oc);
  outputLine('push dword [procTable_pos]','');
  outputLine('add dword [procTable_pos],'+d,'');
  goto oke;
  end;
if (a='procallocend') then begin;
  ...
  d:=getWord(c);
  if (d='') then exit;
  outputLine('pop dword [procTable_data+'+a+'*4]',oc);
  goto oke;
  end;
if (a='procfree') then begin;
  ...
  if (d='') then exit;
  outputLine('pop dword [procTable_data+'+a+'*4]',oc);
  outputLine('sub dword [procTable_pos],'+d,'');
  goto oke;
  end;
...
WriteLn('platform: linux kernel 2.4.2/i386 (nasm syntax)');
a:=paramStr(1);
b:=paramStr(2);
