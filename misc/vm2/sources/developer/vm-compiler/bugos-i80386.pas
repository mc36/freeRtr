...
if (a='jmpc') then begin;
  ...
  a:=getWord(c);
  if (a='') then exit;
  d:='j'+CondCods[CompareSeenSign][i]+' dword offset '+a;
  goto vege;
  end;
...
if (a='procallocbeg') then begin;
  ...
  d:=getWord(c);
  if (d='') then exit;
  outputLine('mov ebp,'+a,oc);
  outputLine('push dword ds:[ebp*4]','');
  outputLine('push dword def:[freeMemPoint]','');
  outputLine('add dword def:[freeMemPoint],'+d,'');
  goto oke;
  end;
...
if (a='procfree') then begin;
  ...
  d:=getWord(c);
  if (d='') then exit;
  outputLine('mov ebp,'+a,'');
  outputLine('pop dword ds:[ebp*4]','');
  outputLine('sub dword def:[freeMemPoint],'+d,'');
  goto oke;
  end;
...
if (a='syscall') then begin;
  ...
  if (a='startup') then begin;
    ...
    o:=o*4;
    outputLine('dummyBuffer equ '+bstr(o+$00),'');
    outputLine('freeMemPoint equ '+bstr(o+$10),'');
    outputLine('consoleSizeX equ '+bstr(o+$14),'');
    outputLine('consoleSizeY equ '+bstr(o+$16),'');
    outputLine('firstFreeMem equ '+bstr(o+$18),'');
    goto oke;
    end;
  c:='--- '+c;
  xtSetPos(ttbl,0);
...
WriteLn('platform: BugOS i80386 (sasm syntax)');
a:=paramStr(1);
b:=paramStr(2);
