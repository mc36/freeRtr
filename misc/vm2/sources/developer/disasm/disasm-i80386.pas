...
{$include disasm-i80386.inc}

Label f1,f2;
Const
  ProggyName='disAsm-i80386 v1.0';
Const
  FieldSep='  ';
  HexCodSiz=10*2;
...
for i:=2 to ParamCount do begin;
  ...
  if (c='use16') then use32:=false;
  if (c='use32') then use32:=true;
  end;
...
if use32 then a:='32' else a:='16';
xtWriteLn(t,'the code is assumed to use '+a+' bit code segment.');
xtWriteLn(t,'');
...
c:=DisAssemblerBinaryCode(b,use32);
if (c='') then c:=#1;
p:=ord(c[1]);
delete(c,1,1);
b:=copy(b,1,p);
...
if (o<>0) then begin;
  ...
  i:=filP+p+i;
  c:=copy(c,1,o-1)+disAsm2hex(i+OrgValue,3);
  end;
a:='';
