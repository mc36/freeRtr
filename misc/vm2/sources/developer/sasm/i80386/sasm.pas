...
Const PrgTxt='sAsm-i80386 v1.8, done by Mc at '#%date' '#%time'.';
...
{$ifdef doMMX}{$include comp_mmx.inc}{$endif}
{$ifdef doSSE}{$include comp_sse.inc}{$endif}
...
if (a='use32') then CurrentCodeStyle:=1;
...
While (tn>0) do begin;
  ...
  {$ifdef doMMX}if (OutOk=False) then CompileOneMmxLine;{$endif}
  {$ifdef doSSE}if (OutOk=False) then CompileOneSseLine;{$endif}
