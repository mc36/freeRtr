...
Var
  ...
  w:Word;
BEGIN;
WriteLn('emulator-m68000 v1.0, done by Mc at '#%date' '#%time'.');
...
case buf[1] of
  ...
  04:begin; {read register values}
    buf[1]:=05;
    move(regs_d,buf[2],32);
    move(regs_a,buf[34],32);
    move(reg_sr,buf[66],sizeof(reg_sr));
    move(reg_pc,buf[70],sizeof(reg_pc));
    move(reg_ss,buf[74],sizeof(reg_ss));
    p:=77;
    end;
  ...
  12:begin; {write register values}
    buf[1]:=13;p:=1;
    move(buf[2],regs_d,32);
    move(buf[34],regs_a,32);
    move(buf[66],reg_sr,sizeof(reg_sr));
    move(buf[70],reg_pc,sizeof(reg_pc));
    move(buf[74],reg_ss,sizeof(reg_ss));
    end;
  ...
  00:begin; {list of registers}
    buf[1]:=01;p:=1;
    for i:=0 to 7 do putReg(buf,p,4,$00,'d'+BStr(i));
    for i:=0 to 7 do putReg(buf,p,4,$00,'a'+BStr(i));
    putReg(buf,p,4,$83,'sr');
    putReg(buf,p,4,$02,'pc');
    putReg(buf,p,4,$01,'ss');
    end;
  02:begin; {register bitmap}
    buf[1]:=03;p:=1;
    move(buf[2],i,sizeof(i));
    case i of
      1..8:putReg(buf,p,0,32,'d'+BStr(i-1));
      9..16:putReg(buf,p,0,32,'a'+BStr(i-1));
      17:begin;
        putReg(buf,p,0,1,'c');
        putReg(buf,p,1,1,'v');
        putReg(buf,p,2,1,'z');
      ...
      18:putReg(buf,p,0,32,'pc');
      37:putReg(buf,p,0,32,'ss');
      end;
    end;
