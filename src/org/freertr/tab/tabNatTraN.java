...
public class tabNatTraN implements Comparable<tabNatTraN> {
    ...
    /**
     * Indicates if this is a reverse entry (for external connections)
     * Used to prevent port pool creation for external IPs
     */
    ...
    /**
     * release resources when the NAT translation is removed
     * This is called when a NAT translation expires or is manually cleared
     */
        ...
        if (protocol == prtTcp.protoNum || protocol == prtUdp.protoNum) {
            ...
            if (debugger.tabNatDebug) {
                logger.info("DEBUG-RELEASE: Starting resource cleanup for NAT entry: " +
                          "protocol=" + protocol +
                          ", origSrcAddr=" + (origSrcAddr != null ? origSrcAddr.toString() : "null") +
                          ", origSrcPort=" + origSrcPort +
                          ", origTrgAddr=" + (origTrgAddr != null ? origTrgAddr.toString() : "null") +
                          ", origTrgPort=" + origTrgPort +
                          ", newSrcAddr=" + (newSrcAddr != null ? newSrcAddr.toString() : "null") +
     ...
     * 
     * @return Array with [sw_packets, sw_bytes, hw_packets, hw_bytes, total_packets, total_bytes]
     */
