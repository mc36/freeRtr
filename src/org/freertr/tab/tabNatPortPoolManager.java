...
/**
 * Singleton class for central management of NAT port pools
 * Each IP address has its own independent port pool
 */
public class tabNatPortPoolManager {
    // Singleton instance
    private static final tabNatPortPoolManager INSTANCE = new tabNatPortPoolManager();
    ...
    /**
     * Class for the master port pool that manages the entire range for a specific IP
     */
    ...
    /**
     * Create a sub-pool for an IP address with a specific port range (default sequence 1000)
     */
    ...
    public Map<addrIP, String> getAllPoolUsages() {
        ...
        try {
            ...
            result.put(masterKey, masterPoolSummary.toString());
            ...
            for (Map.Entry<String, TreeMap<Integer, SubPortPool>> entry : ipToSubPoolsMap.entrySet()) {
                ...
                result.put(poolAddr, sb.toString());
            }
    ...
    /**
     * Get all sub-pools for a specific IP address as a TreeMap of sequence to pool
     * @param addr IP address
     * @return TreeMap of sequence number to SubPortPool, or null if no pools exist
     */
