...
public class userDash {
     ...
     */
    public userDash(pipeSide p, String h) {
        hst = h;
        prm = h.length();
        console = new pipeScreen(p);
    }
    ...
    private void doKeyEnter() {
        ...
        cfgDshbrd ntry = buf.get(cur);
        List<String> lst = ntry.getResult(console.pipe.settingsGet(pipeSetting.tabMod, userFormat.tableMode.normal));
        userEditor v = new userEditor(console, lst, hst + "show dash " + ntry.name, false);
        v.doView();
        doClear();
        doDraw(false);
    }
    ...
    private void doFilter() {
        buf = new ArrayList<cfgDshbrd>();
        max = 0;
        for (int o = 0; o < cfgAll.dshbrds.size(); o++) {
            cfgDshbrd ntry = cfgAll.dshbrds.get(o);
            if (ntry == null) {
                continue;
            }
            int i = ntry.name.length();
            if (max < i) {
                max = i;
            }
            boolean ned = ntry.name.indexOf(flt) >= 0;
            ned |= ntry.getDescr().indexOf(flt) >= 0;
            if (!ned) {
                continue;
            }
            buf.add(ntry);
    ...
    private void doDraw(boolean clr) {
        ...
        console.putCur(prm + flt.length(), console.sizY - 1);
        console.refresh();
    }
    ...
    private void putFooter() {
        ...
        console.putStr(prm, console.sizY - 1, pipeScreen.colBlue, pipeScreen.colBrGreen, false, flt);
    ...
    private void putLine(int ln) {
        ...
        cfgDshbrd ntry = buf.get(lin);
        console.putStr(0, ln + 1, bg, fg, false, ntry.name);
        console.putStr(max + 1, ln + 1, bg, fg, false, ntry.getDescr());
    }
