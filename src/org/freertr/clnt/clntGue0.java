 ...
 * @author matecsaba
 */
public class clntGue0 implements Comparable<clntGue0>, Runnable, prtServP, ifcDn {
    ...
    public String toString() {
        return "gue0 to " + target;
    }

    public int compareTo(clntGue0 o) {
    ...
    public void sendPack(packHolder pck) {
        ...
        pck.getSkip(2);
        i = prtTmux.ethtyp2proto(i);
        if (i < 0) {
            cntr.drop(pck, counter.reasons.badProto);
            return;
        }
        pck.putByte(0, 0); // version
        pck.putByte(1, i); // protocol
    ...
    private void workDoer() {
        ...
        conn = udp.packetConnect(this, fwdIfc, prtL, target, prtR, "gue0", -1, null, -1, -1);
        if (conn == null) {
            return;
        }
        conn.timeout = 120000;
    ...
    public boolean datagramRecv(prtGenConn id, packHolder pck) {
        if (pck.getByte(0) != 0) { // version
            logger.info("got bad version from " + target);
            cntr.drop(pck, counter.reasons.badVer);
            return false;
        }
        int i = pck.getByte(1); // protocol
        ...
        if (pck.msbGetW(2) != 0) {
            logger.info("got bad flags from " + target);
            cntr.drop(pck, counter.reasons.badFlag);
            return false;
        }
        pck.getSkip(size);
        pck.msbPutW(0, i); // ethertypes
        pck.putSkip(2);
        pck.merge2beg();
        cntr.rx(pck);
        upper.recvPack(pck);
