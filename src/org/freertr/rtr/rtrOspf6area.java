...
public class rtrOspf6area implements Comparable<rtrOspf6area>, Runnable {
     ...
     */
    protected synchronized void generateLsa(rtrOspf6lsa lsa, boolean purge, boolean cntnt) {
    ...
    private void calculateSpf() {
            ...
            switch (ntry.lsaType) {
                ...
                case rtrOspf6lsa.lsaRtrInfo:
                    ...
                    for (;;) {
                        ...
                        spf.addAlgo(src, rtrOspfSr.getAlgos(tlv));
                    }
                    break;
        ...
        spf.doWork(null, new rtrOspf6areaSpf(lower.routerID, 0), null);
        if (segrouEna && (lower.segrouLab != null)) {
        ...
        for (int p = 0; p < lower.algos.size(); p++) {
            ...
            spf.doWork(spf.flexExcl(alg.num), new rtrOspf6areaSpf(lower.routerID, 0), null);
            for (int i = 0; i < lower.ifaces.size(); i++) {
                rtrOspf6iface ifc = lower.ifaces.get(i);
                if (ifc == null) {
                    continue;
