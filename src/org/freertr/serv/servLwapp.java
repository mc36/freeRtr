...
import org.freertr.pack.packLwapp;
...
/**
 * lightweight access point protocol (rfc5412) server
 *
 * @author matecsaba
 */
public class servLwapp extends servGeneric implements prtServP {
     ...
     * list of connections
     */
    public tabGen<servLwappConn> conns = new tabGen<servLwappConn>();
    ...
    public final static userFilter[] defaultF = {
        new userFilter("server lwapp .*", cmds.tabulator + "port " + packLwapp.port, null),
        new userFilter("server lwapp .*", cmds.tabulator + "protocol " + proto2string(protoAllDgrm), null),
        new userFilter("server lwapp .*", cmds.tabulator + cmds.negated + cmds.tabulator + "physical-interface", null)
    };
     ...
     */
    public servLwappConn connFind(prtGenConn id, boolean create) {
        servLwappConn ntry = new servLwappConn(id, this);
        if (!create) {
            return conns.find(ntry);
        }
        servLwappConn old = conns.add(ntry);
        if (old != null) {
            return old;
        }
     ...
     * @return connection entry
     */
    public servLwappConn connDel(prtGenConn id) {
        servLwappConn ntry = new servLwappConn(id, this);
    ...
    public void datagramWork(prtGenConn id) {
        servLwappConn ntry = connFind(id, false);
        if (ntry == null) {
            id.setClosing();
            return;
        }
    ...
    public userFormat getShow() {
        ...
        for (int i = 0; i < conns.size(); i++) {
            servLwappConn ntry = conns.get(i);
            if (ntry == null) {
                continue;
            }
...
class servLwappConn implements ifcDn, Comparable<servLwappConn> {

    private servLwapp lower;
    ...
    public int compareTo(servLwappConn o) {
     ...
     */
    public servLwappConn(prtGenConn id, servLwapp parent) {
        conn = id;
        lower = parent;
    }
