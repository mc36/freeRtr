 ...
 * @author matecsaba
 */
public class servEoIp extends servGeneric implements ipPrt {
     ...
     * list of connections
     */
    public tabGen<servEoIpConn> conns = new tabGen<servEoIpConn>();
    ...
    public final static userFilter[] defaultF = {
        new userFilter("server eoip .*", cmds.tabulator + "port " + prtGre.protoNum, null),
        new userFilter("server eoip .*", cmds.tabulator + "protocol " + proto2string(protoAllDgrm), null),
        new userFilter("server eoip .*", cmds.tabulator + cmds.negated + cmds.tabulator + "physical-interface", null),
        new userFilter("server eoip .*", cmds.tabulator + "timeout 60000", null)
    };
    ...
    public void srvHelp(userHelp l) {
        ...
        l.add(null, false, 1, new int[]{2}, "tunnel", "tunnel id");
        l.add(null, false, 2, new int[]{-1}, "<num>", "milliseconds");
    ...
    public void recvPack(ipFwdIface rxIfc, packHolder pck) {
        servEoIpConn ntry = new servEoIpConn(rxIfc, pck.IPsrc, this);
        servEoIpConn old = conns.add(ntry);
        if (old != null) {
            old.doRecv(pck);
            return;
        }
