...
public class rtrOspf6area implements Comparator<rtrOspf6area>, Runnable {
     ...
     * last spf
     */
    protected shrtPthFrst<rtrOspf6areaSpf> spf;
    ...
    private void createExtLsas() {
        ...
        if (defOrigin) {
            ...
            ntry.origin = 111;
            tabRoute.addUpdatedEntry(2, rs, ntry, roumapInto, roupolInto, prflstInto);
        }
        tabRoute.addUpdatedTable(2, rs, lower.routerRedistedU, roumapInto, roupolInto, prflstInto);
    ...
    private void createSrLsas() {
        ...
        for (int i = 0; i < lower.ifaces.size(); i++) {
            ...
            createEprfLsa(seq++, ifc.iface.network, o, p, ifc.metric, rtrOspfSr.putPref(ifc.srIndex, ifc.srNode));
        }
        if (stub || nssa) {
            return;
        }
        for (int i = 0; i < lower.routerComputedU.size(); i++) {
            ...
            createEprfLsa(seq++, ntry.prefix, rtrOspf6lsa.lsaEinterPrf, rtrOspf6lsa.tlvInterPrf, ntry.metric, rtrOspfSr.putPref(ntry.segRoutI, false));
        }
    }
    ...
    private void calculateSpf() {
        ...
        spf = new shrtPthFrst<rtrOspf6areaSpf>(spf);
        for (int i = 0; i < lsas.size(); i++) {
            rtrOspf6lsa ntry = lsas.get(i);
            if (ntry == null) {
                continue;
            ...
            switch (ntry.lsaType) {
                ...
                case rtrOspf6lsa.lsaEprefix:
                    ...
                    for (;;) {
                        ...
                        if ((pref.segRoutI < 1) || (pref.segRoutI >= lower.segrouMax)) {
                            continue;
                        }
                        ...
                        lower.segrouLab[pref.segRoutI].setFwdMpls(9, lower.fwdCore, iface, hop, lab);
