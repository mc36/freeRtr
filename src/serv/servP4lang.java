...
public class servP4lang extends servGeneric implements ifcUp, prtServS {
    ...
    public void srvHelp(userHelping l) {
        ...
        l.add("1 2  export-dynacc             specify dynamic access port range");
        l.add("2 3    <num>                   first id");
        l.add("3 .      <num>                 number of ids");
...
class servP4langVrf implements Comparator<servP4langVrf> {
    ...
    public tabListing<tabAceslstN<addrIP>, addrIP> pbrCfg4;

    public tabListing<tabAceslstN<addrIP>, addrIP> pbrCfg6;

    public tabListing<tabAceslstN<addrIP>, addrIP> pbrCfg4f;
...
class servP4langConn implements Runnable {
    ...
    private tabListing<tabAceslstN<addrIP>, addrIP> doPbrCfg(boolean ipv4, servP4langVrf vrf, ipFwd fwd, tabListing<tabAceslstN<addrIP>, addrIP> old, tabListing<tabAceslstN<addrIP>, addrIP> res) {
        tabListing<tabAceslstN<addrIP>, addrIP> need;
        tabPbrN ntry;
        if (fwd.pbrCfg.size() < 1) {
            ntry = null;
            need = null;
        } else {
            ntry = fwd.pbrCfg.get(0);
            need = ntry.matcher;
        }
        String cmd = "norm";
        String par = "0 0 ";
    ...
    private boolean doMroutes(String afi, int vrf, ipFwdMcast need, ipFwdMcast done, boolean bef) {
        ...
        for (int i = 0; i < dbier.fwds.size(); i++) {
            ...
            servP4langIfc ifc = hop.getVia();
            lower.sendLine("mbierroute" + afi + "_del " + vrf + " " + gid + " " + need.group + " " + need.source + " " + ingr.id + " " + ifc.getMcast(gid).id + " " + hop.id + " " + ntry.label + " " + ifc.id + " " + dbier.srcId + a);
        }
        for (int i = 0; i < nbier.fwds.size(); i++) {
            ...
            String a = doLab5(ntry, tabLabelBier.bsl2msk(ntry.bsl), 0);
            servP4langIfc ifc = hop.getVia();
            lower.sendLine("mbierroute" + afi + "_" + act + " " + vrf + " " + gid + " " + need.group + " " + need.source + " " + ingr.id + " " + ifc.getMcast(gid).id + " " + hop.id + " " + ntry.label + " " + ifc.id + " " + nbier.srcId + a);
            now++;
        }
