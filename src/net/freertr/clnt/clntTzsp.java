...
/**
 * tazman sniffer protocol client
 *
 * @author matecsaba
 */
public class clntTzsp implements Runnable, prtServP, ifcDn {
    ...
    public void sendPack(packHolder pck) {
        ...
        cntr.tx(pck);
        pck.msbPutW(0, 0x0100); // version 1, received
        pck.msbPutW(2, 1); // etthernet
        pck.msbPutW(4, 1); // pad end
        pck.putSkip(size);
        pck.merge2beg();
        pck.putDefaults();
        conn.send2net(pck);
    ...
    private void workDoer() {
        ...
        conn = udp.packetConnect(this, fwdIfc, port, trg, port, "tzsp", null, -1);
        if (conn == null) {
            return;
        }
        conn.timeout = 120000;
    ...
    public boolean datagramRecv(prtGenConn id, packHolder pck) {
        ...
        if (pck.getByte(0) != 1) { // version
            cntr.drop(pck, counter.reasons.badVer);
            return false;
        }
        //int dir = pck.getByte(1); // direction
        if (pck.msbGetW(2) != 1) { // ethernet
            cntr.drop(pck, counter.reasons.badTyp);
            return false;
        }
        if (pck.msbGetW(4) != 1) { // pad end
