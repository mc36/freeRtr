...
public class clntMtrack implements Runnable, prtServS {
    ...
    private void doRebind() {
        ...
        udp.streamListen(this, new pipeLine(4096, true), null, port, null, 0, 0, "mtrck", null, -1);
        if (!grp.isMulticast()) {
            return;
        }
        for (int i = 0; i < pers.size(); i++) {
    ...
    private void doConfig() {
        ...
        pipeSide pipe = udp.streamConnect(new pipeLine(4096, true), ifc, 0, cfgTrg.get(0), port, "mtrck", null, -1);
        if (pipe == null) {
            return;
        }
        pipe.timeout = 5000;
    ...
    private void doReport() {
        ...
        if (grp.isMulticast()) {
            pipeSide pipe = udp.streamConnect(new pipeLine(4096, true), ifc, 0, grp, port, "mtrck", null, -1);
            if (pipe == null) {
                return;
            }
            pipes.add(pipe);
            ...
            for (int i = 0; i < pers.size(); i++) {
                pipeSide pipe = udp.streamConnect(new pipeLine(4096, true), ifc, 0, pers.get(i).adr, port, "mtrck", null, -1);
                if (pipe == null) {
                    continue;
                }
                pipes.add(pipe);
